...
def retrieve_relevant_docs(query: str, top_k: int = 5):
    ...
    return [st.session_state.chunked_documents[i] for i in indices[0]]
...
if selected_incident_types:
    ...
    if 'documents' in st.session_state and 'chunked_documents' not in st.session_state:
        if st.button("Chunk Documents"):
            with st.spinner("Chunking documents..."):
                st.session_state.chunked_documents = chunk_documents(st.session_state.documents)
            st.success(f"Documents chunked successfully! Total chunks: {len(st.session_state.chunked_documents)}")
    elif 'chunked_documents' in st.session_state:
        st.success("Documents are already chunked.")
    
    # Step 3: Create embeddings
    if 'chunked_documents' in st.session_state and 'embeddings' not in st.session_state:
        if st.button("Create Embeddings"):
            with st.spinner(f"Creating embeddings using {st.session_state.embedding_model}..."):
                ...
                st.session_state.tokenizer, st.session_state.model, st.session_state.embeddings = create_embeddings(st.session_state.chunked_documents, model_name)
    ...
    if st.button("Extract Incident Types"):
        if 'chunked_documents' not in st.session_state:
            st.warning("Please chunk the documents first.")
        else:
            with st.spinner("Extracting incident types..."):
                st.session_state.incident_types = extract_incident_types(st.session_state.chunked_documents, selected_incident_types)
    ...
    if st.button("Count Incidents"):
        if 'incident_types' not in st.session_state:
            ...
            with st.spinner("Counting incidents..."):
                counts = count_incident_types(st.session_state.chunked_documents, st.session_state.incident_types)
    ...
    if st.button("Analyze Patterns"):
        if 'incident_types' not in st.session_state:
            ...
            with st.spinner("Analyzing patterns..."):
                analysis_results = analyze_patterns(st.session_state.chunked_documents, st.session_state.incident_types)
